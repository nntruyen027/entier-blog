...
interface IProps {
  ...
  onSave: ({ name, description, image }) => void;
  value?: { id: number; name: string; description: string; image?: string };
}
    ...
    <Backdrop sx={(theme) => ({ color: '#fff', zIndex: theme.zIndex.drawer + 1 })} open={open}>
      <Paper sx={{ minWidth: '40rem' }}>
        <div className='w-full relative top-0 left-0 text-xl font-bold py-3'>
          <h3 className='lowercase first-letter:uppercase'>{t('update', { value: t('brand') })}</h3>
          <CloseIcon
        ...
        <div className='p-3 w-full flex gap-3 justify-end'>
          <Button variant='outlined' onClick={() => onSave({ name, description, image: files?.[0]?.url })}>
            {t('save')}
          </Button>
        </div>
      </Paper>
