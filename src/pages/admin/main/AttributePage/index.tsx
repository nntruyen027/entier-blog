...
} from '~/redux/attribute/slice';

const AttributePage = () => {
  const { t } = useTranslation();
  const { attributes, pageCount, rowCount } = useSelector((state: RootState) => state.attribute);
  ...
  useEffect(() => {
    dispatch(getAttributesStart({ size: pagination.pageSize, page: pagination.pageIndex }));
  }, [dispatch, pagination]);

  const handleSave = ({ name, description, type }) => {
    dispatch(createAttributeStart({ name, description, type }));
    setOpenCreate(false);
  };

  const handleUpdate = ({ name, description, type }) => {
    dispatch(
      updateAttributeStart({
        id: currentUser.id,
        name,
  ...
  useEffect(() => {
    if (confirmDelete) {
      dispatch(deleteAttributeStart(currentUser.id));
