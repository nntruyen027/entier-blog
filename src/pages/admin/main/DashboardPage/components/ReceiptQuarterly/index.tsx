...
import { doQuarterlyStatisticsStart } from '~/redux/receipt/slice';
import { QuarterlyStatisticsComponent } from '~/pages/admin/main/DashboardPage/components';

const Component = () => {
  const dispatch = useDispatch();
  const { quarterlyStatistics } = useSelector((state: RootState) => state.receipt);
  ...
  return (
    <QuarterlyStatisticsComponent lines={productList} data={chartData} name={'Số lượng sản phẩm mua vào theo quý'} />
  );
};
